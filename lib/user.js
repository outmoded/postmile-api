            ...
            if (request.payload.username) {
                internals.checkUsername(self.db, request.payload.username, function (err, lookupUser) {
                    ...
                    if (!err ||
                        ...
                        return reply(typeof err === 'string' ? Hapi.Error.badRequest('Invalid username: ' + err) : err);
                    }
    ...
    handler: function (request, reply) {
        ...
        exports.load(this.db, request.auth.credentials.user, function (err, user) {
            ...
            if (request.payload.action === 'add') {
                ...
                if (isFound) {
                    return reply(Hapi.Error.badRequest('Address already present'));
                }
                ...
                self.db.count('user', { 'emails.address': address }, function (err, count) {
                    ...
                    if (count) {
                        return reply(Hapi.Error.badRequest('Email already assigned to another account'));
                    }
                ...
                if (!isFound) {
                    return reply(Hapi.Error.notFound('No such email address'));
                }

                // Check if current primary

                if (i === 0) {
                    return reply(Hapi.Error.badRequest('Address already primary'));
                }

                // Check if verified

                if (!user.emails[i].isVerified) {
                    return reply(Hapi.Error.badRequest('Email must be verified before made primary'));
                }
                ...
                if (i === 0) {
                    return reply(Hapi.Error.badRequest('Cannot remove primary address'));
                }

                if (user.emails.length <= 1) {
                    return reply(Hapi.Error.badRequest('Cannot remove the only address present'));
                }
                ...
                if (user.emails[i].isVerified) {
                    return reply(Hapi.Error.badRequest('Account already verified'));
                }
    ...
    handler: function (request, reply) {
        ...
        var validate = function () {
            ...
            if (!request.payload.network &&
                ...
                return reply(Hapi.Error.badRequest('Must include either a network id or email address'));
            }
            ...
            self.db.count('user', { 'emails.address': email }, function (err, count) {
                ...
                if (count) {
                    return reply(Hapi.Error.badRequest('Email address already linked to an existing user'));
                }
        ...
        var validateNetwork = function () {
            ...
            if (request.payload.network.length !== 2) {
                return reply(Hapi.Error.badRequest('Bad network array size'));
            }
            ...
            if (!networkId) {
                return reply(Hapi.Error.badRequest('Empty network id'));
            }

            if (['twitter', 'facebook', 'yahoo'].indexOf(network) === -1) {
                return reply(Hapi.Error.badRequest('Unknown network'));
            }
            ...
            self.db.count('user', criteria, function (err, count) {
                ...
                if (count) {
                    return reply(Hapi.Error.badRequest(request.payload.network[0].replace(/^\w/, function ($0) { return $0.toUpperCase(); }) + ' account already linked to an existing user'));
                }
        ...
        var createAccount = function () {
            ...
            self.db.insert('user', user, function (err, items) {
                ...
                if (items.length !== 1 ||
                    ...
                    return reply(Hapi.Error.internal('Failed to retrieve new user id', user));
                }
    ...
    handler: function (request, reply) {
        ...
        exports.load(this.db, request.params.id, function (err, user) {
            ...
            if (user[request.params.network]) {
                return reply(Hapi.Error.badRequest('Network already linked'));
            }
            ...
            self.db.count('user', criteria, function (err, count) {
                ...
                if (count) {
                    return reply(Hapi.Error.badRequest('Network id already linked to another user'));
                }
...
exports.unlink = {
    ...
    handler: function (request, reply) {
        ...
        exports.load(this.db, request.params.id, function (err, user) {
            ...
            if (!user[request.params.network]) {
                return reply(Hapi.Error.badRequest('Account not linked'));
            }
            ...
            if ((!user.emails || !user.emails.length) &&
                ...
                return reply(Hapi.Error.badRequest('Cannot remove last linked account'));
            }
    ...
    handler: function (request, reply) {
        ...
        if (request.params.type === 'username') {
            internals.checkUsername(this.db, request.params.id, function (err, lookupUser) {
                ...
                if (!lookupUser) {
                    return reply(typeof err === 'string' ? Hapi.Error.badRequest(err) : err);
                }
            ...
            if (!Email.checkAddress(request.params.id)) {
                return reply(Hapi.Error.badRequest('Invalid email address'));
            }
            ...
            return reply(Hapi.Error.badRequest('Unknown network type'));
        }
    }
};
    ...
    handler: function (request, reply) {
        ...
        var check = function () {
            ...
            Project.unsortedList(self.db, request.auth.credentials.user, function (err, projects, owner, notOwner) {
                ...
                if (notOwner.length) {
                    return reply(Hapi.Error.badRequest('Must first leave all projects'));
                }
                ...
                if (owner.length > 1) {                 // Multiple own projects
                    return reply(Hapi.Error.badRequest('Must first delete all projects'));
                }
                ...
                if (owner[0].participants.length !== 1) {
                    return reply(Hapi.Error.badRequest('Must first delete project (has participants)'));
                }
                ...
                Task.count(self.db, owner[0]._id, function (err, count) {
                    ...
                    if (count) {
                        return reply(Hapi.Error.badRequest('Must first delete project'));
                    }
