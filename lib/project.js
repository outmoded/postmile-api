...
exports.post = {
    validate: {
        ...
        payload: {
            title: Joi.string(),
            date: Joi.string().regex(Utils.dateRegex).emptyOk(),
            time: Joi.string().regex(Utils.timeRegex).emptyOk(),
            place: Joi.string().emptyOk()
        }
    },
    handler: function (request, reply) {
        ...
        exports.load(this.db, request.params.id, request.auth.credentials.user, true, function (err, project, member) {
            ...
            if (Object.keys(request.payload).length > 0) {
                if (request.query.position) {
                    return reply(Boom.badRequest('Cannot include both position parameter and project object in body'));
                }
                ...
                return reply(Boom.badRequest('Missing position parameter or project object in body'));
            }
        });
    }
};
...
exports.participants = {
    validate: {
        ...
        payload: {
            participants: Joi.array().includes(Joi.string()),     //!! ids or emails
            names: Joi.array().includes(Joi.string())
        }
    },
    handler: function (request, reply) {
        ...
        var add = function () {
            ...
            if (request.query.message.match('://')) {
                return reply(Boom.badRequest('Message cannot contain links'));
            }
        ...
        var process = function () {
            ...
            if (!request.payload.participants &&
                ...
                return reply(Boom.badRequest('Body must contain a participants or names array'));
            }
                ...
                if (request.payload.participants) {
                    ...
                    User.load(self.db, request.auth.credentials.user, function (err, user) {
                        ...
                        User.find(self.db, request.payload.participants, function (err, users, emailsNotFound) {
                            ...
                            var prevParticipants = Hoek.mapToObject(project.participants, 'id');
                            ...
                            var prevPids = Hoek.mapToObject(project.participants, 'email');
                            ...
                            if (change.$pushAll.participants.length <= 0) {
                                return reply(Boom.badRequest('All users are already project participants'));
                            }
...
exports.uninvite = {
    validate: {
        payload: {
            participants: Joi.array().required().includes(Joi.string())
        }
    },
    handler: function (request, reply) {
        ...
        var uninvite = function () {
            ...
            exports.load(self.db, request.params.id, request.auth.credentials.user, true, function (err, project, member) {
                ...
                if (!exports.isOwner(project, request.auth.credentials.user)) {
                    return reply(Boom.badRequest('Not an owner'));
                }
                ...
                if (request.params.user) {
                    ...
                    if (request.auth.credentials.user === request.params.user) {
                        return reply(Boom.badRequest('Cannot uninvite self'));
                    }
                    ...
                    if (!uninvitedMember) {
                        return reply(Boom.notFound('Not a project participant'));
                    }
                    ...
                    for (var i = 0, il = request.payload.participants.length; i < il; ++i) {
                        ...
                        if (request.auth.credentials.user === removeId) {
                            error = Boom.badRequest('Cannot uninvite self');
                            break;
                        }
                        ...
                        if (!uninvited) {
                            error = Boom.notFound('Not a project participant: ' + removeId);
                            break;
                        }
                    ...
                    if (uninvitedMembers.length === 0) {
                        error = Boom.badRequest('No members to remove');
                    }
                    ...
                    return reply(Boom.badRequest('No participant for removal included'));
                }
            });
        };
...
exports.join = {
    handler: function (request, reply) {
        ...
        exports.load(this.db, request.params.id, request.auth.credentials.user, false, function (err, project, member) {
            ...
            if (!member.isPending) {
                return reply(Boom.badRequest('Already a member of the project'));
            }
...
exports.load = function (db, projectId, userId, isWritable, callback) {
    ...
    db.get('project', projectId, function (err, item) {
        ...
        if (!item) {
            return callback(err || Boom.notFound());
        }
        ...
        if (!member) {
            return callback(Boom.forbidden('Not a project member'));
        }

        if (isWritable &&
            item.isPending) {

            return callback(Boom.forbidden('Must accept project invitation before making changes'));
        }
    ...
    Task.userTaskList(db, project._id, (isPid ? 'pid:' + userId : userId), function (err, tasks) {
        ...
        if (tasks.length > 0) {
            ...
            if (isPid === false) {
                ...
                if (!member.isPending) {
                    return callback(Boom.badRequest('Cannot remove pid user with task assignments'));
                }
