...
internals.Manager.prototype.initialize = function (server) {
    ...
    server.route([
        { method: 'POST', path: '/stream/{id}/project/{project}', config: this.subscribe() },
        { method: 'DELETE', path: '/stream/{id}/project/{project}', config: this.unsubscribe() }
    ]);
    ...
    return {
        handler: function (request, reply) {
            ...
            if (!manager.socketsById[request.params.id] ||
                ...
                return reply(Hapi.Error.notFound('Streamer not found'));
            }

            if (!manager.socketsById[request.params.id].userId) {
                return reply(Hapi.Error.badRequest('Streamer not initialized'));
            }
