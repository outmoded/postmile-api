...
exports.app = {
    ...
    handler: function (request, reply) {
        ...
        this.db.queryUnique('client', { name: request.params.id }, function (err, client) {
            ...
            Utils.removeKeys(client, ['algorithm', 'key', 'scope']);
            return reply(client);
        });
    }
};
...
exports.login = {
    validate: {
        payload: {
            type: Joi.string().valid('id', 'twitter', 'facebook', 'yahoo', 'email').required(),
    ...
    handler: function (request, reply) {
            ...
            self.db.query('grant', { user: user.id, app: appId }, function (err, items) {
                ...
                self.db.insert('grant', newGrant, function (err, items) {
                    ...
                    if (items.length !== 1 ||
                        ...
                        return reply(Boom.internal('Failed to add new grant'));
                    }
        ...
        var issue = function (appId, grantId, ext) {
            ...
            Oz.ticket.rsvp({ id: appId }, { id: grantId }, self.vault.ozTicket, {}, function (err, rsvp) {
...
exports.loadApp = function (db) {
    ...
    return function (id, callback) {
        ...
        if (!id) {
            return callback(Boom.internal('Missing client id'));
        }

        db.get('client', id, function (err, client) {

            if (err || !client) {
                return callback(err || Boom.unauthorized('Unknown client'));
            }
...
exports.loadGrant = function (db) {
    ...
    return function (grantId, callback) {
        ...
        db.get('grant', grantId, function (err, item) {
            ...
            if (err || !item) {
                return callback(err || Boom.unauthorized('Unknown grant'));
            }

            User.load(db, item.user, function (err, user) {

                if (err || !user) {
                    callback(err || Boom.unauthorized('Invalid grant'));
                }
...
exports.validateMessage = function (env, message, authorization, callback) {
    ...
    var credentialsFunc = Oz.server.credentialsFunc(env.vault.ozTicket, {});
    Oz.hawk.server.authenticateMessage(env.config.server.api.host, env.config.server.api.port, message, authorization, credentialsFunc, {}, function (err, credentials) {
        ...
        if (err) {
            return callback(Boom.notFound('Invalid ticket'));
        }
